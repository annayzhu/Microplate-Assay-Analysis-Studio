...
面向酶标仪/多功能微孔板读数仪的一体化分析工作台。当前版本为 v0.6.8，使用真实 Thermo Scientific Varioskan LUX / SkanIt XML、XLSX 和旧版 VICTOR XLS 数据开发。
...
1. 所有已开放实验类型共用同一套输入路径：仪器结果、Excel 粘贴/模板、可复现项目文件；可同时选择多个独立仪器文件，也可把后续日期的板追加到当前项目，导入后统一经过多板预览与逐板确认。
...
9. 细胞活性/增殖分析页以“导出结果 Excel”作为唯一正式结果入口，工作簿整合导出说明、生物学汇总、技术复孔汇总、孔级数据和可读板布局；不再提供孔级、技术复孔或汇总的独立 CSV 按钮。技术复孔汇总按 biological replicate 显示孔级 mean，生物学汇总再按实验条件显示 biological mean、SD 和 SEM；跨板标准化准备表和标准化结果仍按各自用途单独导出。
...
15. 板布局导入确认区采用紧凑任务流；板图和批量注释使用等高、相互独立的滚动视口，注释标题与应用按钮固定在面板内。批量注释仍可折叠，折叠不会清除当前选择或尚未应用的填写内容。
16. 多板项目在“板图与实验注释”和“分析与导出”页面都显示当前板名称、来源和逐板标签，可直接切换各板；未应用注释会在切板或返回上一步前受到保护。各板切换时不会沿用上一板的孔选择或汇总行筛选。
17. 不在单日导出中预先把主值归一化到 Control；需要比较增殖曲线时，可下载标准化准备表自行处理，或显式启用跨板 baseline normalization。系统按板扣除 blank、在每个 Bio 内先合并技术复孔，再按精确时间点和明确 Bio ID 计算派生结果。
18. Baseline normalization 支持组内或参考组基线、匹配重复比值、均值比值加误差传播及明确标注的固定基线缩放，并可选择 fold/percent 与 SEM/95% CI 展示。原始 blank-corrected 汇总和显著性计算不被派生标准化覆盖；标准化配置、方法、原始 baseline 统计量、来源板和警告随 CSV/项目文件保存。
19. 分析页图表使用独立的深墨绿数据舞台、青玉柱、暖色误差线和高对比标签；颜色、阴影和密度均由集中 token 管理，数值、误差线、坐标和选择行为不因视觉处理而改变。
...
- 多个独立仪器文件的一次性选择、追加/替换决策、板 ID 防碰撞及既有项目状态保留。
- 四板项目在导入、板图和分析页的标签切换、当前板身份、未应用草稿保护、返回上一步和窄屏溢出检查。
- 6/12/24/48/96/384 孔读数模板生成与回读。
- 板图框选、追加选择、逐孔取消、缩放、板图与注释独立滚动、批量草稿保留与离开保护。
- 当前板布局导出、CSV 引号回读、跨板型预警、生物学重复清空选项及原始读数只读保护。
- CCK-8 的空白扣除、技术复孔合并、生物学汇总，以及未归一化主值与相对 Control 派生列的导出边界。
- 结果 Excel 的导出说明、生物学汇总、技术复孔汇总、孔级数据和板布局工作表，以及两级 mean、点选范围、来源字段与下载回读。
