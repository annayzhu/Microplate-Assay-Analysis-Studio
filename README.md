...
面向酶标仪/多功能微孔板读数仪的一体化分析工作台。当前版本为 v0.6.6，使用真实 Thermo Scientific Varioskan LUX / SkanIt XML、XLSX 和旧版 VICTOR XLS 数据开发。
...
9. 细胞活性/增殖分析页以“导出结果 Excel”作为唯一正式结果入口，工作簿整合导出说明、生物学汇总、孔级数据和可读板布局；不再提供孔级、技术复孔或汇总的独立 CSV 按钮。技术复孔仍作为统计计算的内部中间层；跨板标准化准备表和标准化结果仍按各自用途单独导出。
