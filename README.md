...
面向酶标仪/多功能微孔板读数仪的一体化分析工作台。当前版本为 v0.6.4，使用真实 Thermo Scientific Varioskan LUX / SkanIt XML、XLSX 和旧版 VICTOR XLS 数据开发。
...
15. 板布局导入确认区采用紧凑任务流；批量注释面板可折叠，在桌面端为窄侧栏、窄屏端为横向栏，折叠不会清除当前选择或尚未应用的填写内容。
16. 多板项目在“板图与实验注释”页显示当前板名称、来源和逐板标签，可直接切换各板；未应用注释会在切板或返回上一步前受到保护。分析页也提供返回板图的明确入口。
...
- 四板项目在导入页和板图页的标签切换、当前板身份、未应用草稿保护、返回上一步和窄屏溢出检查。
