...
16. 不在单日导出中预先把主值归一化到 Control；需要比较增殖曲线时，可下载标准化准备表自行处理，或显式启用跨板 baseline normalization。系统按板扣除 blank、在每个 Bio 内先合并技术复孔，再按精确时间点和明确 Bio ID 计算派生结果。
17. Baseline normalization 支持组内或参考组基线、匹配重复比值、均值比值加误差传播及明确标注的固定基线缩放，并可选择 fold/percent 与 SEM/95% CI 展示。原始 blank-corrected 汇总和显著性计算不被派生标准化覆盖；标准化配置、方法、原始 baseline 统计量、来源板和警告随 CSV/项目文件保存。
